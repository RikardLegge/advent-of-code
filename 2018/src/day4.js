export function day4a() {
  const SleepStart = Symbol("Sleep Start");
  const SleepEnd = Symbol("Sleep End");
  const ShiftStart = Symbol("Shift Start");

  const log = [];
  const rawLog = input.split('\n');
  for (const entry of rawLog) {
    const [_, year, month, day, hour, minute, sleepStart, sleepEnd, newGuard] = /\[(\d+)-(\d+)-(\d+) (\d+):(\d+)] (?:(falls asleep)|(wakes up)|Guard #(\d+) begins shift)/.exec(entry);
    const date = new Date(Number(year), Number(month) - 1, Number(day), Number(hour), Number(minute));
    if (newGuard) {
      log.push({id: Number(newGuard), date, action: ShiftStart});
    } else if (sleepStart) {
      log.push({date, action: SleepStart});
    } else if (sleepEnd) {
      log.push({date, action: SleepEnd});
    }
  }
  ...
  for (const entry of log) {
    const {id, date, action} = entry;
    if (guard && action === SleepEnd) {
      const startMinute = startTime.getMinutes();
      const minutes = (date - startTime) / 1000 / 60;
      for (let i = 0; i < minutes; i++) {
        const now = startMinute + i;
        const prevCount = guard.minutesSleeping[now] || 0;
        guard.minutesSleeping[now] = prevCount + 1;
        guard.totalSleep++;
      }
    }
    if (action === ShiftStart) {
      if (!guards[id]) {
        guards[id] = {id, minutesSleeping: {}, totalSleep: 0};
      }
      guard = guards[id];
    }
    startTime = date;
  ...
  const sleepyGuard = Object.values(guards).sort((a, b) => b.totalSleep - a.totalSleep)[0];
  const minuteMostlySleeping = Object.entries(sleepyGuard.minutesSleeping).sort((a,b)=>b[1] - a[1])[0][0];
  return sleepyGuard.id * minuteMostlySleeping;
}

export function day4b() {
  const SleepStart = Symbol("Sleep Start");
  const SleepEnd = Symbol("Sleep End");
  const ShiftStart = Symbol("Shift Start");
  ...
  for(const guard of Object.values(guards)) {
    let maxCount = 0;
    let maxMinute = 0;
    for(const [minute, count] of Object.entries(guard.minutesSleeping)) {
      if(count > maxCount) {
        maxCount = count;
        maxMinute = minute;
      }
    }
    guard.maxSleepMinute = {minute: Number(maxMinute), count: maxCount};
  }

  const sleepyGuard = Object.values(guards).sort((a, b) => b.maxSleepMinute.count - a.maxSleepMinute.count)[0];
  return sleepyGuard.id * sleepyGuard.maxSleepMinute.minute;
}

const input = `[1518-07-03 23:58] Guard #2437 begins shift
